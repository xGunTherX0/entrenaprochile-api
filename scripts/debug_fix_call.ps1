...
try {
    ...
    Write-Host "Posting fix_schema_v2..."
    # Use a minimal JSON body
    $response = Invoke-WebRequest -Method Post -Uri "$base/api/admin/fix_schema_v2" -Headers @{ Authorization = "Bearer $token" } -Body '{}' -ContentType 'application/json' -UseBasicParsing -ErrorAction Stop
